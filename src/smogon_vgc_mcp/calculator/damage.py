...
def parse_ev_string(evs: str) -> dict[str, int]:
    """Parse EV string to dict.

    Args:
        evs: EV string like "252/0/4/0/0/252" or "252 HP / 4 Def / 252 Spe"

    Returns:
        Dict with hp, atk, def, spa, spd, spe keys
    """
    if "/" in evs and not any(c.isalpha() for c in evs.split("/")[0]):
        # Format: "252/0/4/0/0/252"
        parts = [int(x.strip()) for x in evs.split("/")]
        if len(parts) == 6:
            return {
                "hp": parts[0],
                "atk": parts[1],
    ...
    for part in evs.split("/"):
        ...
        for stat_name, stat_key in stat_map.items():
            if stat_name in part.lower():
                try:
                    value = int("".join(c for c in part if c.isdigit()))
...
def parse_iv_string(ivs: str | None) -> dict[str, int]:
    """Parse IV string to dict.

    Args:
        ivs: IV string like "31/31/31/31/31/31" or "31 HP / 0 Atk / 31 Spe"

    Returns:
        Dict with hp, atk, def, spa, spd, spe keys (defaults to 31)
    """
    if ivs is None:
        return {"hp": 31, "atk": 31, "def": 31, "spa": 31, "spd": 31, "spe": 31}

    if "/" in ivs and not any(c.isalpha() for c in ivs.split("/")[0]):
        # Format: "31/31/31/31/31/31"
        parts = [int(x.strip()) for x in ivs.split("/")]
        if len(parts) == 6:
            return {
                "hp": parts[0],
                "atk": parts[1],
    ...
    # Format: "31 HP / 0 Atk / 31 Spe" - start with 31s, override specified
